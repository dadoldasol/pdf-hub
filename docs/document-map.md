...
|---|---|---|---|---|
| `README.md` | 문서 전체 진입점 | 문서 목록, 읽는 순서, MVP 목표 | 전체 팀, LLM/Codex | 어떤 문서를 먼저 볼지 판단 |
...
| `mvp-scope.md` | MVP 범위 고정 | 포함/제외 기능, API, 데이터 모델 | 전체 팀 | 일정 중 scope creep 방지 |
...
| `backend/ingestion-pipeline.md` | 업로드 처리 흐름 정의 | job 상태, 파일 저장, 파이프라인 단계 | 백엔드/AI | worker 구현 |
...
| 문서 | 목적 | 주요 내용 | 대상 | 구현 활용 |
|---|---|---|---|---|
| `llm/extraction-pipeline.md` | LLM 처리 흐름 정의 | 요약, 엔티티, 관계, 분류 | AI 개발자 | extractor 구현 |
...
| 문서 | 목적 | 주요 내용 | 대상 | 구현 활용 |
|---|---|---|---|---|
| `operations/local-dev.md` | 로컬 개발 환경 정의 | env, docker compose, seed data | 개발자 | 온보딩 |
